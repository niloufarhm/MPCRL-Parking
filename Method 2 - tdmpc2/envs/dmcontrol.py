...
class DMControlWrapper:
	...
	def rand_act(self):
		"""Return a random action as a torch tensor."""
		import torch
		act = self.env.action_spec().sample()
...
def make_env(cfg):
	...
	"""
	print("!!!!!!!!!!!!!!!!!!!!!!!!!!> USING envs.dmcontrol.make_env (DMControlWrapper with rand_act)")

	domain, task = cfg.task.replace('-', '_').split('_', 1)
	domain = dict(cup='ball_in_cup', pointmass='point_mass').get(domain, domain)
	if (domain, task) not in suite.ALL_TASKS:
		raise ValueError('Unknown task:', task)
	assert cfg.obs in {'state', 'rgb'}, 'ThisA task only supports state and rgb observations.'
