...
def parse_cfg(cfg: OmegaConf) -> OmegaConf:
	...
	cfg.work_dir = str(Path(hydra.utils.get_original_cwd()) / 'logs' / cfg.task / str(cfg.seed) / cfg.exp_name)
	...
	if hasattr(cfg, "tasks") and isinstance(cfg.tasks, dict):
		task_cfg = cfg.tasks.get(cfg.task, None)
		if task_cfg is not None:
			# Copy relevant task-specific fields to top level
			for key in ["obs_shape", "action_dim", "episode_length"]:
				if key in task_cfg:
					setattr(cfg, key, task_cfg[key])
